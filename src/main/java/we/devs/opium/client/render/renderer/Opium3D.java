...
/**
 * Updated version of Renderer3D
 * @see me.x150.renderer.render.Renderer3d
 */
@SuppressWarnings("unused")
public class Opium3D {
    ...
    private static void translateMatrix(Vec3d inPos, MatrixStack inMatrixStack) {
        Camera camera = client.gameRenderer.getCamera();
        Vec3d camPos = camera.getPos();
        inMatrixStack.translate(
                Math.max(inPos.x, camPos.x) - Math.min(inPos.x, camPos.x),
                Math.max(inPos.y, camPos.y) - Math.min(inPos.y, camPos.y),
                Math.max(inPos.z, camPos.z) - Math.min(inPos.z, camPos.z)
        );
    }
    ...
    public interface MatrixModifier {
        void modifyMatrix(MatrixStack stack);
    }

    public static void renderEdged(MatrixStack stack, Color colorFill, Color colorOutline, Vec3d start, Vec3d dimensions, MatrixModifier modifier) {
        Matrix4f matrix = stack.peek().getPositionMatrix();
        float[] fill = getColor(colorFill);
        float[] outline = getColor(colorOutline);

//        translateMatrix(start, stack);
        modifier.modifyMatrix(stack);
