...
public class DamageUtils {
    ...
    private static float explosionDamage(LivingEntity target, Vec3d explosionPos, float power, boolean predictMovement, RaycastFactory raycastFactory) {
        ...
        if (target instanceof PlayerEntity player && mc.getNetworkHandler().getPlayerListEntry(player.getUuid()) != null && mc.getNetworkHandler().getPlayerListEntry(player.getUuid()).getGameMode() == GameMode.CREATIVE && !(player instanceof FakePlayer)) return 0f;
    ...
    private static float modifyAttackDamage(LivingEntity attacker, LivingEntity target, ItemStack weapon, DamageSource damageSource, float damage) {
       ...
       Util.getEnchantments(weapon, enchantments);
        float enchantDamage = 0f;

        int sharpness =Util.getEnchantmentLevel(enchantments, Enchantments.SHARPNESS);
        if (sharpness > 0) {
            enchantDamage += 1 + 0.5f * (sharpness - 1);
        }

        int baneOfArthropods =Util.getEnchantmentLevel(enchantments, Enchantments.BANE_OF_ARTHROPODS);
        ...
        int impaling =Util.getEnchantmentLevel(enchantments, Enchantments.IMPALING);
        if (impaling > 0 && target.getType().isIn(EntityTypeTags.SENSITIVE_TO_IMPALING)) {
            enchantDamage += 2.5f * impaling;
        }

        int smite =Util.getEnchantmentLevel(enchantments, Enchantments.SMITE);
        ...
        if (attacker instanceof PlayerEntity playerEntity) {
            ...
            if (weapon.getItem() instanceof MaceItem item) {
                ...
                if (bonusDamage > 0f) {
                    int density =Util.getEnchantmentLevel(weapon, Enchantments.DENSITY);
    ...
    private static float protectionReduction(LivingEntity player, float damage, DamageSource source) {
        ...
        for (ItemStack stack : player.getAllArmorItems()) {
            Object2IntMap<RegistryEntry<Enchantment>> enchantments = new Object2IntOpenHashMap<>();
            Util.getEnchantments(stack, enchantments);

            int protection = Util.getEnchantmentLevel(enchantments, Enchantments.PROTECTION);
            if (protection > 0) {
                damageProtection += protection;
            }

            int fireProtection = Util.getEnchantmentLevel(enchantments, Enchantments.FIRE_PROTECTION);
            if (fireProtection > 0 && source.isIn(DamageTypeTags.IS_FIRE)) {
                damageProtection += 2 * fireProtection;
            }

            int blastProtection = Util.getEnchantmentLevel(enchantments, Enchantments.BLAST_PROTECTION);
            if (blastProtection > 0 && source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                damageProtection += 2 * blastProtection;
            }

            int projectileProtection = Util.getEnchantmentLevel(enchantments, Enchantments.PROJECTILE_PROTECTION);
            if (projectileProtection > 0 && source.isIn(DamageTypeTags.IS_PROJECTILE)) {
                damageProtection += 2 * projectileProtection;
            }

            int featherFalling = Util.getEnchantmentLevel(enchantments, Enchantments.FEATHER_FALLING);
