package we.devs.opium.client.utils.render.framebuffers;
 ...
 * @author x150
 */
public class OpiumOutlineFramebuffer extends Framebuffer {
    private static OpiumOutlineFramebuffer instance;

    private OpiumOutlineFramebuffer(int width, int height) {
        super(false);
    ...
    private static OpiumOutlineFramebuffer obtain() {
        if (instance == null) {
            instance = new OpiumOutlineFramebuffer(MinecraftClient.getInstance().getFramebuffer().textureWidth,
    ...
    public static void use(Runnable r) {
        ...
        RenderSystem.assertOnRenderThreadOrInit();
        OpiumOutlineFramebuffer buffer = obtain();
    ...
    public static void draw(float radius, Color outlineColor, Color innerColor) {
        ...
        ShaderManager.OUTLINE_SHADER.render(MinecraftClient.getInstance().getRenderTickCounter().getTickDelta(true));
     ...
     * Partial draw
     */
    public static void use$init() {
        mainBuffer = MinecraftClient.getInstance().getFramebuffer();
        RenderSystem.assertOnRenderThreadOrInit();
        buffer = obtain();
        if (buffer.textureWidth != mainBuffer.textureWidth || buffer.textureHeight != mainBuffer.textureHeight) {
            buffer.resize(mainBuffer.textureWidth, mainBuffer.textureHeight, false);
    ...
    static OpiumOutlineFramebuffer buffer;
    /**
     * End partial draw
     */
    public static void use$end() {
        buffer.endWrite();
